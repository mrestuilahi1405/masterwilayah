# -*- coding: utf-8 -*-
"""
Master Wilayah Indonesia - V24.0 (Custom Logo BPS)
--------------------------------------------------
Update:
- Mengganti icon globe default dengan logo BPS kustom.
- Menambahkan konfigurasi 'StaticFiles' agar FastAPI bisa membaca
  file gambar lokal (logo_bps.png) dari direktori yang sama.

Prasyarat:
- Pastikan file gambar (misal: 'logo_bps.png') ada di folder yang sama
  dengan script ini.

Usage:
$ python data_master.py
"""

import logging
import re
import os # Import OS untuk cek file
...
from fastapi.staticfiles import StaticFiles # PENTING: Import ini untuk file statis (gambar)

# ==========================================
# 1. KONFIGURASI & SECURITY
# ==========================================
...
# --- KONFIGURASI LOGO ---
# Ganti 'logo_bps.png' dengan nama file logo Anda yang sebenarnya.
LOGO_FILENAME = "logo_bps.png" 

# PENTING: Mounting direktori saat ini (".") ke path "/static".
# Ini membuat semua file di folder ini bisa diakses via URL /static/namafile
...
def load_data() -> None:
    global df_master
    # Cek keberadaan logo (opsional, untuk info di log)
    if os.path.exists(LOGO_FILENAME):
        logger.info(f"✅ File logo '{LOGO_FILENAME}' ditemukan.")
    else:
        logger.warning(f"⚠️ File logo '{LOGO_FILENAME}' TIDAK ditemukan di direktori ini. Gambar mungkin broken.")
...
<head>
    ...
    <meta name="description" content="Cari kode wilayah administrasi Indonesia (Provinsi, Kabupaten, Kecamatan, Desa) dengan cepat dan akurat.">
    <meta name="theme-color" content="#1e40af">
    <link rel="icon" type="image/png" href="/static/{logo_filename}">
    
    <meta property="og:type" content="website">
    <meta property="og:title" content="Master Wilayah Indonesia">
    <meta property="og:description" content="Database Kode Wilayah Administrasi Indonesia Terlengkap.">
...
<body class="min-h-screen flex flex-col bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100 transition-colors duration-300">
        ...
        <div class="w-full max-w-[96%] mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16 items-center">
                <div class="flex items-center gap-3.5">
                    <img src="/static/{logo_filename}" alt="Logo BPS" class="w-11 h-auto p-1 bg-white/95 backdrop-blur rounded-lg shadow-md border border-white/20">
    ...
    <main class="flex-grow w-full max-w-[96%] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div id="loading" class="hidden flex flex-col items-center justify-center py-16">
            <div class="animate-spin rounded-full h-10 w-10 border-[3px] border-slate-100 dark:border-slate-700 border-t-blue-600 mb-4"></div>
            <span class="text-sm font-medium text-slate-500 dark:text-slate-400">Memproses...</span>
        </div>

        <div id="hasil" class="min-h-[200px] transition-opacity duration-300"></div>
    </main>
    ...
    <script>
        ...
        function restoreFromURL() {
            ...
            ['prov', 'kabupaten', 'kecamatan', 'desa'].forEach(id => {
                const val = params.get(id); if (val) document.getElementById(id).value = val;
            });
            loadResults(); // Fix: Always trigger initial load
        }
...
""".replace("{logo_filename}", LOGO_FILENAME) # Inject filename into template
